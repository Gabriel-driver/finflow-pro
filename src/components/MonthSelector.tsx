...
export function MonthSelector({ currentDate, prevMonth, nextMonth }: MonthSelectorProps) {
  const label = currentDate.toLocaleDateString("pt-BR", { month: "long", year: "numeric" });

  return (
    <div className="flex items-center gap-3">
      <button onClick={prevMonth} className="p-2 rounded-lg hover:bg-muted/30 text-muted-foreground hover:text-foreground transition-colors active:scale-95">
      ...
      <button onClick={nextMonth} className="p-2 rounded-lg hover:bg-muted/30 text-muted-foreground hover:text-foreground transition-colors active:scale-95">
